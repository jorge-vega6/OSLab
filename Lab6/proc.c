 ...
 */
  ...
  if (!rdy_head[q]) {		/* add to empty queue */
	  ...
	  if (rp->deadline == 0) { //if deadline is 0 then place at the end of the line
		  ...
		  rp->p_nextready = NULL;		/* mark new end */
	  } else if (rdy_head[q]->deadline > rp->deadline) {  // compare priorities
		  //rp's priority is higher
		  rp->p_nextready = rdy_head[q]; //set rp's next to current head
		  ...
		  while(next != NULL) { //enter this loop if there are more than one processes
			  if (next->deadline > rp->deadline) {
				  prev->p_nextready = rp;
				  rp->p_nextready = next;
				  ...
				  if (next == NULL) { //if we found the last one, just append last
					  rdy_tail[q]->p_nextready = rp;		/* chain tail of queue */
					  rdy_tail[q] = rp;				/* set new queue tail */
					  rp->p_nextready = NULL;		/* mark new end */
				  }
